...
{
	unsigned int nr_cpus = num_online_uniso_cpus();
	int max_cpus = pm_qos_request(PM_QOS_MAX_ONLINE_CPUS) ? : CONFIG_NR_CPUS;
...
static void runnables_work_func(struct work_struct *work)
{
	...
	if (action > 0)
		cpu = cpumask_next_zero(0, cpu_unisolated_mask);
	else if (action < 0)
